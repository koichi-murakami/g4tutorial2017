...
//    fScoringVol = logVol_BGO;
    ...
    = new G4MultiFunctionalDetector("BGO");     // SDの名前 "BGO"
  G4SDManager::GetSDMpointer()->AddNewDetector(BGODetector);
    G4VPrimitiveScorer* primitive;
    primitive = new G4PSEnergyDeposit("Edep");      // 物理量の名前 Edep
    BGODetector->RegisterPrimitive(primitive);
    SetSensitiveDetector("LogVol_BGO",BGODetector);  // /BGO/Edepでヒットコレクションが作られる
